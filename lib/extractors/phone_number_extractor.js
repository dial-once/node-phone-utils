  ...
  return {
    ...
    getCountryCode: function getCountryCode(gPhoneNumber){
      return new Promise(function (resolve, reject) {
        ...
        var loggMessage = loggingFunc('get-country-code');
        logger.debug(loggMessage('Calling get-country-code for: ' + gPhoneNumber));

        if (gPhoneNumber) {
          var cCode = gPhoneNumber.getCountryCodeOrDefault();
          logger.info(loggMessage('Country code is' + cCode));
          return resolve(cCode);
        } else {
