 ...
 * @example
 * var phoneNumbersUtils = require('node-phone-utils').getInstance();
 */
...
var PhoneNumberUtils = function PhoneNumberUtils(options) {
  ...
  if (!_.isObject(pUtilsOptions) || _.isArray(pUtilsOptions)) {
    ...
    pnValidator = PNValidator.getInstance();
    pnExtractor = PNExtractor.getInstance();
    pnFormatter = PNFormatter.getInstance();
  }
   ...
   *  },
    ...
    hlrLookup: function hlrLookup(e164Number, provider) {
      ...
      if (! InputValidator.isValidHLRLookupProvider(_provider)) {
       ...
       return _provider.hlrLookup(e164Number);
      }
    },
 ...
 * @param [options] Optional options object where you ca supply logger, provider options
