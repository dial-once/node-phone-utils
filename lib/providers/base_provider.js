...
require('dotenv');
var PhoneNumberValidator = require('../validators/phone_number_validator');
var HLRResultExtractor = require('../extractors/hlr_result_extractor');
var getLogger = require('../helpers/logger_options_parser').getLoggerFromOptions;
/**
 * Base HRL lookup provider, a reference interface for implementation of hlr lookup providers
...
function BaseProvider(name, username, password, logger) {
  ...
  this.validator = PhoneNumberValidator.getInstance({logger: logger});
  this.hlrExtractor = HLRResultExtractor.getInstance({logger: logger});
}
